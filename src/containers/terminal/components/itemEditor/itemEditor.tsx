...
const ItemEditor: React.FC<EditorProps> = ({ orderItemId, order, products, services }) => {
  ...
  const handleRemove = () => services.updateCurrentOrder(getUpdatedOrder({ ...item, quantity: 0 }));
  const handleApply = () => services.updateCurrentOrder(getUpdatedOrder());
  const handleCancel = () => services.setCurrentItem(0);
